# 🔑 Getting Your Google API Key - Google Environmental Suite

### *for Hubitat Elevation Drivers*
...
- [🛠️ Step-by-Step Instructions](#step-by-step-instructions)
  ...
  - [🪜 Step 4 – Enable the Environmental APIs](#step-4--enable-the-environmental-apis)
...
The **Google Environmental API Suite** drivers require a Google API key to fetch local data, such as `ragweedPollenLevel`, `currentTemperature`, and `airQualityIndex` for your **Hubitat Elevation** hub. This beginner-friendly guide walks you through obtaining an API key from the Google Cloud Console (GCC), setting up **Billing** (required even for the free tier), and securing the key with usage limits. The process takes 10–15 minutes for new users, with clear steps to avoid errors like **"Billing not linked"** or **"API not enabled."**
...
**Next Steps:** After obtaining your API key, follow the **Installation Guide** to install the driver code and the **Setup & Configuration Guide** to configure the device.

---

<h2 id="why-you-need-an-api-key">🔑 Why You Need an API Key</h2>

The API key authenticates your **Hubitat Elevation** driver to Google’s servers, enabling secure access to environmental forecasts. Google requires **Billing** for all APIs, but the free tier (e.g., 10,000 requests/month for Pollen Essentials) covers typical usage.

- **Purpose:** Unlocks weather, pollen, and air quality data for **Webcore** automations, **Dashboards**, and health insights.
- **Free Tier:** Sufficient for residential polling intervals (15+ minutes).
- **Cost:** Zero for typical use; pay-as-you-go beyond the free tier with volume discounts.  
...
**Purpose:** A Project acts as the required container for your API key, its security restrictions, and its **Quotas**. It isolates your resources for better organization.
...
- If you already have a **Billing Account**: Link it to your project and skip to **Step 4 – Enable the Environmental APIs**.
...
<h3 id="step-4-enable-the-environmental-apis">🪜 Step 4 – Enable the Environmental APIs</h3>

**Purpose:** You must explicitly enable the specific API services in your Google Cloud **Project** that correspond to the **Hubitat Elevation** drivers you intend to use. Without enabling the specific service, the driver will return an "API Not Enabled" error in the **Logs**.

**Instructions:**

1. Navigate to the left menu → **APIs & Services → Library**.  
2. Search for and **ENABLE** only the APIs required for your specific installation:
   - **Pollen API:** Required for the Pollen Forecaster driver.
   - **Air Quality API:** Required for the Air Quality Index driver.
   - **Weather API (Google Maps Platform):** Required for the Weather/Local Conditions driver.
3. For each selected API, click **ENABLE**. Verify the button changes from **ENABLE** to **MANAGE**, indicating the service is active for your **Project**.

<h3 id="step-5-create-an-api-key">🪜 Step 5 – Create an API Key</h3>

**Purpose:** The API key is the unique token used by your **Hubitat Elevation** driver to authenticate every request to Google's servers.
...
2. **App Name:** Enter a clean name like **"Hubitat Environmental Services"**.  
3. **User Support Email:** Select your email from the dropdown.  
4. Click **Save** or **Create**. (Note: You do NOT need to add logos or authorized domains for private **Hubitat Elevation** use).

<h3 id="step-6-restrict-the-api-key">🪜 Step 6 – Restrict the API Key</h3>

**Purpose:** Restricting the key is the most important security measure. It ensures the key can only be used for the specific APIs you enabled in **Step 4** and blocks fraudulent charges if the key is accidentally exposed.

**Instructions:**

1. Navigate to **APIs & Services → Credentials**. Click your API key name.  
2. Under **API restrictions**, select **Restrict key**.  
3. In the **Select APIs** dropdown, select ONLY the APIs you enabled for your drivers:
   - **Google Maps Pollen API** (if using Pollen Forecaster)
   - **Air Quality API** (if using Air Quality Index)
   - **Weather API** (if using Weather/Local Conditions)
4. Click **SAVE**.  
5. Verify **Key restrictions** shows the correct count of API restrictions based on your selection.

<h3 id="step-7-quotas-and-limits-cost-control">🪜 Step 7 – Quotas and Limits (Cost Control)</h3>

**Purpose:** Setting a **Quotas** limit acts as a hard cap on your spending. If you exceed the set limit, Google will return an error instead of charging you for unexpected usage.
...
- Set your **Hubitat Elevation** polling interval to 15 or 30 minutes in the **Preferences** section of the **Device Detail Page** to stay safely within the free tier.
...
- **Hubitat Errors:** See **Logs** in **Hubitat Elevation** and refer to the Troubleshooting & Verification Guide.

---

<h2 id="summary">📝 Summary</h2>

This guide provides complete, step-by-step instructions for obtaining and securing a Google API key for **Hubitat Elevation** environmental drivers. By following Steps 1–7, users create an account, set up a project, link **Billing**, enable the APIs, create a key, apply restrictions, and configure **Quotas** to prevent unexpected charges. Following these steps ensures safe, secure, and efficient API usage within Google’s free tier.
...
|---------|------------|--------|------------------------------------------------------------------------------------------------------|
| **1.35**| **2026-03-28** | **DBQ** | **Updated Step 4 and Step 6 to utilize conditional logic for individual API enablement.**            |
